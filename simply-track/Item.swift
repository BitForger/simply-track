...
enum SimplyTrackSchemaV3: VersionedSchema {
    ...
    final class FoodCatalogItem {
        ...
        var defaultAmountDescription: String
        var caloriesPerDefaultAmount: Double
        // Optional storage avoids a SwiftData lightweight-migration crash casting nil to Bool on pre-existing rows.
        var isUserAddedRawValue: Bool?
        ...
        init(
            ...
            self.defaultAmountDescription = defaultAmountDescription
            self.caloriesPerDefaultAmount = caloriesPerDefaultAmount
            self.isUserAddedRawValue = isUserAdded
        }

        var isUserAdded: Bool {
            get { isUserAddedRawValue ?? false }
            set { isUserAddedRawValue = newValue }
        }
    }
...
enum SimplyTrackMigrationPlan: SchemaMigrationPlan {
    static var schemas: [any VersionedSchema.Type] {
        [SimplyTrackSchemaV1.self, SimplyTrackSchemaV2.self, SimplyTrackSchemaV3.self]
    }
...
typealias FoodEntry = SimplyTrackSchemaV3.FoodEntry
typealias FoodCatalogItem = SimplyTrackSchemaV3.FoodCatalogItem
typealias UserProfile = SimplyTrackSchemaV3.UserProfile
